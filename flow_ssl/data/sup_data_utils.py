...
def make_sup_data_loaders(
    ...
    if dataset == "celeba":
        ...
        '''
        train_dataset = torchvision.datasets.CelebA(root=os.path.join(path, 'train'), transform=transforms.Compose([transforms.Resize(image_size), transforms.CenterCrop(image_size), transforms.ToTensor()]), split='train')
        
        test_dataset = torchvision.datasets.CelebA(root=os.path.join(path, 'test'), transform=transforms.Compose([transforms.Resize(image_size), transforms.CenterCrop(image_size), transforms.ToTensor()]), split='test')
        '''
        train_loader = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=batch_size,
