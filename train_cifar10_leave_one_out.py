...
transform_train = transforms.Compose([transforms.ColorJitter(brightness=0.5, hue=0.3),
                                    ...
                                    transforms.ToTensor()])
'''
transform_train = transforms.Compose([transforms.RandomCrop(32, padding=4, pad_if_needed=True),
                                    transforms.RandomHorizontalFlip(p=0.5),
                                    transforms.RandomVerticalFlip(p=0.5),
                                    transforms.ToTensor()])
...
for epoch in range(start_epoch, start_epoch + args.num_epochs + 1):
    ...
    fig, ax = plt.subplots()
    plt.hist(test_ll[test_ll > test_ll_percentile], color='r', label='In-distribution', alpha=0.5, density=True)    
    plt.hist(ood_ll[ood_ll > ood_ll_percentile], color='b', label='Out-of-distribution', alpha=0.5, density=True)
    ...
    fig, ax = plt.subplots()
    plt.hist(test_grad_norm_list[test_grad_norm_list > test_grad_norm_list_percentile], color='r', label='In-distribution', alpha=0.5, bins=50, log=True)    
    plt.hist(ood_grad_norm_list[ood_grad_norm_list > ood_grad_norm_list_percentile], color='b', label='Out-of-distribution', alpha=0.5, bins=50, log=True)
