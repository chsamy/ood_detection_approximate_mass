...
def compute_tnr_at_95_tpr(targets, preds):
    '''
    ...
    Outputs: TNR @ 95% TPR
    '''
    fpr, tpr, _ = roc_curve(targets, preds, pos_label=1) #get false positive rae for varying levels of true positive rate
...
parser.add_argument('--output_folder', type=str, default=None, required=False, metavar='PATH',
                help='path to results (default: None)')
...
parser.add_argument('--id_class', type=str, default='dog', required=False, help='In-distribution class in the FashionMNIST datastet. By default the dog class is considered ID.')
...
for beta in np.arange(0.55, 1, 0.01):
    ...
    tnr95tpr = compute_tnr_at_95_tpr(targets, preds)
